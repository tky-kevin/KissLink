 ...
 *
 * <p>{@link WifiDirectManager} 門面與 {@link GroupOwnerController}、{@link ClientConnector}、 {@link
 * GoDetectionPoller} 都透過此物件存取共享的可變狀態：系統服務、P2P Channel （會在斷線時重建）、連線狀態機（LiveData）、超時計時器與重入守衛。
 *
 * <p>把這些共享資源集中於此，是為了讓上述控制器各自只負責單一角色流程， 而不需互相持有對方的欄位。本類別<b>不</b>含任何 GO/Client 角色邏輯。
 *
 * <p>minSdk=29 (Q), so {@code @RequiresApi} annotations are omitted throughout this module (would
 * be flagged by lint as {@code ObsoleteSdkInt}).
 */
class WifiDirectCore {
    ...
    /**
     * 暫時性 P2P 失敗（BUSY/ERROR）的重試設定。切換傳輸/接收太快時，框架可能還在 拆除上一場群組而回 BUSY(2)/ERROR(0)；短延遲後重試即可成功，毋須讓使用者重來。
     *
     * <p>注意：當本機<b>熱點開著</b>時的 BUSY 並非「暫時性、會沉澱」的失敗（ap0+wlan0 介面組合已耗盡），重試再多次也不會成功，只會讓注定失敗的連線拖更久才報錯。
    ...
    // ── LiveData（對外，由門面轉發） ───────────────────────────
    final MutableLiveData<ConnectionState> stateLd = new MutableLiveData<>(ConnectionState.IDLE);
    final MutableLiveData<GroupCredential> credentialLd = new MutableLiveData<>();
    final MutableLiveData<String> errorLd = new MutableLiveData<>();

    /**
     * 同步重入守衛:createGroupAsGO/connectAsClient 進行中為 true,擋住重疊呼叫的競態 (state 用 postValue 非同步更新,光靠 state
     * 檢查擋不住同一輪的第二次呼叫)。
     */
    ...
    /**
     * 狀態機的<b>唯一</b>外部入口：各角色控制器投遞 {@link WifiDirectEvent} 意圖，由此處 集中決定目標狀態並寫入。非法（未知）來源狀態時記錄到
     * FlightRecorder 供診斷，但不阻擋轉移 （保持與舊有直接 {@code setState} 呼叫相同的行為）。
     */
    ...
    static String reasonToString(int reason) {
        switch (reason) {
            case WifiP2pManager.P2P_UNSUPPORTED:
                return "裝置不支援 Wi-Fi P2P (1)";
            case WifiP2pManager.ERROR:
                return "內部錯誤 (0)";
            case WifiP2pManager.BUSY:
                return "系統繁忙，請稍後再試 (2)";
            default:
                return "未知錯誤 (code=" + reason + ")";
        }
    }
