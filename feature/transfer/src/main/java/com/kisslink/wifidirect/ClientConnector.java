...
/**
 * Client 端（接收方）連線流程：靜默加入 GO 的 Wi-Fi Direct 群組、偵測群組形成、 並把 P2P {@link Network} 綁定至目前 Process。
 *
 * <h3>為何用 WifiP2pManager.connect() 而非 WifiNetworkSpecifier？</h3>
 *
 * {@code WifiNetworkSpecifier + requestNetwork()} 是「請系統幫我連到某個 Wi-Fi」的 API， 系統強制顯示授權視窗（Android 10+
 * 安全設計，無法繞過）。<br>
 * {@code WifiP2pManager.connect()} 是 Wi-Fi Direct 原生 P2P API， 加入已知 Group 不需使用者確認，完全靜默。
 */
class ClientConnector {
    ...
    /**
     * 連線偵測輪詢偵測到群組已形成時，回呼門面的 {@link WifiDirectManager#onConnectionInfoAvailable} 以共用同一條處理路徑
     * （與系統廣播觸發時的行為完全一致）。
     */
     ...
     *
     * <p>連線成功後由 {@link WifiDirectManager#onConnectionInfoAvailable} 觸發 {@link #onGroupFormed}，以取得
     * {@link Network} 物件並綁定至目前 Process。
     */
    ...
    void connectAsClient(@NonNull GroupCredential credential) {
        if (core.starting || core.currentState() != ConnectionState.IDLE) {
            Log.w(
                    TAG,
                    "connectAsClient skipped, starting="
                            + core.starting
                            + " state="
                            + core.currentState());
            return;
        ...
        core.p2pManager.removeGroup(
                ...
                new WifiP2pManager.ActionListener() {
                    ...
                    public void onFailure(int r) {
                        Log.d(
                                TAG,
                                "removeGroup before connect: failed "
                                        + r
                                        + " (expected if no group)");
                        doConnectAsClient(config);
                    }
                });
    }
                    ...
                    public void onFailure(int reason) {
                        ...
                        if (WifiDirectCore.isTransientP2pError(reason)
                            ...
                            Log.w(
                                    TAG,
                                    "connect() failed "
                                            + WifiDirectCore.reasonToString(reason)
                                            + ", transient retry "
                                            + (attempt + 1)
                                            + "/"
    ...
    /**
     * isGroupOwner=false 且 groupFormed=true → 本裝置已成功加入 GO 的群組。 由門面 {@link
     * WifiDirectManager#onConnectionInfoAvailable} 的 Client 分支呼叫。
     */
    ...
    /**
     * P2P 群組形成後，靜默取得網路物件並綁定至 Process。 使用不含 NetworkSpecifier 的 requestNetwork() ——
     * 只是「要求一個符合條件的已存在網路」， 不會觸發系統 UI。
     */
                ...
                new ConnectivityManager.NetworkCallback() {
                    ...
                    public void onAvailable(@NonNull Network network) {
                        ...
                        android.net.NetworkCapabilities nc =
                                core.connManager.getNetworkCapabilities(network);
                        boolean isP2p =
                                nc != null
                                        && !nc.hasCapability(
                        ...
                        if (!isP2p) {
                            Log.w(
                                    TAG,
                                    "Ignoring non-P2P match (Wi-Fi AP/STA) "
                                            + network
