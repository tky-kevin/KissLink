 ...
 *
 * <p>為何重要：Single-Channel-Concurrency（SCC）會把 Wi-Fi Direct 的 P2P 群組頻段
 * 釘在 GO 既有 STA 連線的頻段上；STA 落在 2.4GHz 時，P2P 也被迫 2.4GHz → 吞吐遠低於 5GHz。
 * 把協商頻段分類出來，可在診斷（FlightRecorder dump）中解釋「為何傳輸偏慢」。
 *
 * <p>純函式、不碰 Android framework，便於單元測試。門檻 {@link #BAND_5GHZ_MIN_MHZ}
 * 與 {@code WifiDirectManager.canHostFastGroup} 共用，確保「快頻段」判定單一真相。
 */
public final class WifiBand {
    ...
    public enum Band { BAND_2_4GHZ, BAND_5GHZ, UNKNOWN }
