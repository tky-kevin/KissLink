...
/**
 * GO 端（傳送方）流程：建立 Wi-Fi Direct 群組、產生/取回連線憑證、進入 {@link ConnectionState#HOSTING} 並啟動 {@link
 * GoDetectionPoller} 等待 Client 加入。
 */
class GroupOwnerController {
    ...
    /**
     * 以 Group Owner 身份建立 Wi-Fi Direct 群組（傳送方呼叫）。 成功後 {@link WifiDirectManager#getCredential()} 會發射
     * {@link GroupCredential}， 狀態進入 {@link ConnectionState#HOSTING}。
     */
    ...
    void createGroupAsGO() {
        if (core.starting || core.currentState() != ConnectionState.IDLE) {
            Log.w(
                    TAG,
                    "createGroupAsGO skipped, starting="
                            + core.starting
                            + " state="
                            + core.currentState());
            return;
    ...
    private void doCreateGroup(final int attempt) {
        ...
        WifiP2pConfig config =
                new WifiP2pConfig.Builder().setNetworkName(ssid).setPassphrase(passphrase).build();
                        ...
                        core.p2pManager.createGroup(
                                ...
                                new WifiP2pManager.ActionListener() {
                                    ...
                                    public void onSuccess() {
                                        ...
                                        Log.d(
                                                TAG,
                                                "createGroup (legacy) success → requesting group info");
                                    ...
                                    public void onFailure(int reason2) {
                                        ...
                                        if (WifiDirectCore.isTransientP2pError(reason2)
                                            ...
                                            Log.w(
                                                    TAG,
                                                    "createGroup failed "
                                                            + WifiDirectCore.reasonToString(reason2)
                                                            + ", transient retry "
                                                            + (attempt + 1)
                                                            + "/"
                                                            + WifiDirectCore
                                                                    .P2P_TRANSIENT_MAX_RETRY);
                                            // 先盡力移除殘留群組讓框架沉澱，延遲後重建（startTimeout 由 doCreateGroup 重設）
                                            core.p2pManager.removeGroup(
                                                    core.getChannel(), core.noopListener);
    ...
    /** 呼叫 requestGroupInfo 取得系統實際採用的 SSID（可能微調）與 Passphrase， 若 API 回傳 null 則 fallback 使用本機產生的值。 */
    @SuppressLint("MissingPermission")
