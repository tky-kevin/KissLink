...
/**
 * Sender thread: takes {@link SendItem}s from a queue, writes HELLO + META + CHUNK×N + COMPLETE to
 * the socket via pipelined prefetch I/O.
 *
 * <p>Created by {@link PeerConnection#start()} and runs on its own thread. Exits when it encounters
 * {@link PeerConnection#STOP} in the queue.
 *
 * <p>On socket error mid-transfer, saves a {@link PeerConnection.PendingSend} to {@code
 * pendingSendRef} so {@link FileTransferService} can inject it into the next connection for
 * same-session resume.
 */
final class PeerSender implements Runnable {
    ...
    interface FrameWriter {
        void writeFrame(byte[] header, @Nullable byte[] payload, int off, int len)
                throws IOException;
    }
    ...
    private void sendHello() throws IOException {
        ...
        if (!hasProfile) {
            frameWriter.writeFrame(
                    TransferProtocol.encodeHeader(TransferProtocol.makeHello()), null, 0, 0);
            return;
        }
        try {
            ...
            if (selfAvatarThumb != null)
                p.put(
                        ...
                        android.util.Base64.encodeToString(
                                selfAvatarThumb, android.util.Base64.NO_WRAP));
            byte[] payload = p.toString().getBytes(StandardCharsets.UTF_8);
            frameWriter.writeFrame(
                    TransferProtocol.encodeHeader(
                            TransferProtocol.makeHelloWithProfile(payload.length)),
                    payload,
                    0,
                    payload.length);
        } catch (Exception e) {
    ...
    private void sendOne(PeerConnection.OutItem oi, long resumeFrom) {
        ...
        try {
                            ...
                            () -> {
                                try (InputStream in = openItemInput(item)) {
                                    ...
                                    while (true) {
                                        ...
                                        if (r <= 0) {
                                            ready.put(PeerConnection.CHUNK_EOF);
                                            return;
                                        }
                                        c.len = r;
                                        ready.put(c);
            ...
            try {
                while (true) {
                    ...
                    TransferProtocol.Header ch =
                            TransferProtocol.makeDataChunk(0, offset, c.len, crc);
                    frameWriter.writeFrame(TransferProtocol.encodeHeader(ch), c.data, 0, c.len);
                    offset += c.len;
                    sent += c.len;
                    lastOffset[0] = offset;
                    // doneBytes counts from file start (resumeFrom + sent) for accurate progress
                    // display
                    emitProgress(
                            true,
                            item.name,
            ...
            // COMPLETE
            frameWriter.writeFrame(
                    TransferProtocol.encodeHeader(TransferProtocol.makeComplete(0)), null, 0, 0);
            ok = true;
            if (index >= count - 1)
                emitAllDone(true, item.name, size, item.itemType, batchId, count);
