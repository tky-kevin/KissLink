...
/**
 * 單一 JVM 內的「假 peer」傳輸框架：用本機 TCP socket 接出一對相連的端點， 不需要兩台實機就能讓傳送/接收邏輯在<b>真實 stream</b>上互通。
 *
 * <p>為何用真 socket 而非記憶體 pipe：要驗的正是 framing 在 TCP 上的行為—— 連續多幀的排序、TCP 分段下 {@code readFully} 的重組、{@link
 * BufferedOutputStream} 緩衝 + flush 的時機。這些是 {@link TransferProtocol} 純編解碼單元測試覆蓋不到的。
 *
 * <p>幀 I/O 刻意對齊生產碼：寫端等同 {@code PeerConnection.writeFrame} （header(64B) + 可選 payload，逐幀 flush）；讀端等同
 * {@code PeerReceiver.readFully} + 依封包型別推導 payload 長度。如此跑過的幀序列與真實 Wi-Fi Direct 通道一致。
 *
 * <p>用法：
 *
 * <pre>{@code
...
final class LoopbackPeers implements AutoCloseable {
    ...
    static final class Peer {
        ...
        /**
         * 讀出一幀：先 readFully 64B header 解碼，再依型別讀對應 payload。 對齊 {@code PeerReceiver} 的解析——payload
         * 長度來源： HELLO / DATA_CHUNK → {@code chunkLen}；FILE_META → {@code metaLen}；其餘無 payload。
         */
