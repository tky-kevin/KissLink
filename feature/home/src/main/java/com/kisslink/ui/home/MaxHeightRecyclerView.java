 ...
 *
 * <p>搭配 {@code layout_height="wrap_content"} 使用：以 {@code AT_MOST(maxHeight)} 量測，
 * 內容小於上限時精確貼合內容高度（含 ItemDecoration 的上下留白 → 上下對稱），
 * 內容超過上限時固定在上限並可捲動（露出淡出邊）。
 *
 * <p>取代「以固定每列估計值算死高度」的做法——後者會因估計值大於實際列高，
 * 把多餘空間累積在底部，造成下緣留白比上緣寬。
 */
