...
public class PermissionHelper {
    ...
    /**
     * 「碰觸 → 連線」流程實際用到的執行期權限子集（藍牙側通道 + Wi-Fi Direct）。 與 {@link #getRequiredPermissions()}
     * 不同：刻意不含媒體/通知權限——那些被拒不該擋住配對。
     */
     ...
     * 個人熱點（SoftAP）是否開啟。
     *
     * <p>為何重要：Wi-Fi 晶片的並存介面額度多半只有兩個；熱點開著時 {@code ap0}(AP) + {@code wlan0}(STA) 已佔滿，Wi-Fi Direct 想再開
     * {@code p2p} 介面會被 HAL 拒絕 （{@code bestIfaceCreationProposal is null}）→ createGroup/connect 一律回
     * BUSY(2)。 重試無效，因為熱點不關介面就不會空出來。故配對前先擋下並提示關閉。
     *
     * <p>無公開 API 可查自身 SoftAP 狀態，改用反射呼叫隱藏的 {@code WifiManager#isWifiApEnabled()}。 反射被新版系統擋下或拋例外時一律回
     * {@code false}（保守：偵測不到就不擋，避免誤擋正常配對）。
     */
     ...
     *
     * <p>API 33+ 改用 {@code NEARBY_WIFI_DEVICES} 執行期權限，P2P 不再依賴定位服務； 但 API 32 及以下（含本專案 minSdk 29）的
     * Wi-Fi P2P 探索/連線仍要求<b>定位服務主開關</b> 開啟——權限授予但主開關關閉時，{@code discoverPeers}/{@code connect} 會靜默失敗、
    ...
    /**
     * 定位服務主開關是否開啟。用 {@link android.location.LocationManager#isLocationEnabled()} （API 28+，本專案
     * minSdk 29 下恆可用）。取不到服務時保守回 {@code false}。
     */
    ...
    /** 回傳第一個尚未開啟的無線電（依 NFC → 藍牙 → Wi-Fi 順序）；全部已開啟回 {@code null}。 供配對前置檢查決定要提示使用者開啟哪一項。 */
    @Nullable
