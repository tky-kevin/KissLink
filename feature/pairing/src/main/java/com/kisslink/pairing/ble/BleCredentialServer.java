 ...
 *
 * <ul>
 *   <li>central 把自己的 token 寫進 {@link BleConstants#CHAR_PEER_TOKEN} → {@link Callback#onPeerToken}。
 *   <li>若本機當選 GO:{@link #publishCredential} 透過 notify 把憑證送給 central。
 *   <li>若對方當選 GO:對方 write 憑證進 {@link BleConstants#CHAR_CREDENTIAL} → {@link
...
public class BleCredentialServer {
        ...
        this.timeoutRunnable =
                () -> {
                    FlightRecorder.event(
                            TAG,
                            "server timeout: no central connected within window "
    ...
    public void start(@NonNull PairingToken localToken) {
        ...
        if (gattServer == null) {
            callback.onError("無法開啟 GATT server");
            return;
        }
        ...
        BluetoothGattCharacteristic peerTokenChar =
        ...
        seq(
                "GATT server up + advertising nonce="
                        + localToken.nonceB64()
    ...
    @SuppressLint("MissingPermission")
    @SuppressWarnings(
            "deprecation") // setValue/notifyCharacteristicChanged 的新多載需 API 33;minSdk 29 保留舊 API
