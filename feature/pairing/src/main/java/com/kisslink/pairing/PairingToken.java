...
/**
 * 碰觸配對的「bootstrap token」——在 NFC latch 當下單向交換的極小資料, <b>不含</b> Wi-Fi Direct 群組憑證(憑證稍後走 BLE 側通道)。
 *
 * <h3>用途</h3>
 *
 * <ul>
 *   <li><b>session id</b>:{@link #nonce} 同時當 BLE 廣播鍵與場次識別。
 *   <li><b>GO 選舉</b>:{@link #goIntent} 為主、nonce 為輔的決定性比較 ({@link #shouldBeGroupOwner})。雙方各持兩份 token
 *       時會算出一致結果。
 *   <li><b>冷啟動</b>:序列化成 {@code kisslink://pair?...} deep link, 讓對方 App 未開時由 OS NFC dispatch 啟動(情境
 *       1)。
 * </ul>
 *
 * <p>NFC 是單向讀:一次 latch 只有 reader 讀到 tag 的 token。reader 之後透過 BLE 把自己的 token 回送給 tag,兩邊才都湊齊兩份 token
 * 做選舉。
 *
 ...
 *
 * <ul>
 *   <li><b>身分(一場配對內必須凍結)</b>:{@link #nonce}(會合鍵/場次 id)與 {@link #goIntent} (選舉權重)。一旦經 NFC/BLE
 *       曝光給對方,對方就以這份快照做掃描與 GO 選舉; 本機若中途換掉任一個,對方掃不到(nonce 漂移)或雙方選舉結果不再反對稱(goIntent 漂移)。
 *   <li><b>酬載(可隨時刷新)</b>:{@link #canHost5G}(反映當下 Wi-Fi 狀態)與 {@link #deviceName}
 *       (名片姓名)。應在<b>閒置時</b>刷新,使下一場曝光的值最新;但同一場曝光後同樣需凍結 (canHost5G 亦參與選舉)。
 * </ul>
 *
 * <p>因此「刷新酬載」絕不可連帶換掉身分——用 {@link #withPayload} 保留 nonce/goIntent、只換酬載, 而非 {@link
 * #create}(後者會重抽身分)。身分的單一穩定來源見 {@link LocalPairing}。
 */
public final class PairingToken {
    ...
    private static final int B64_FLAGS = Base64.URL_SAFE | Base64.NO_PADDING | Base64.NO_WRAP;
    ...
    /**
     * 本機若當 GO 是否能在 5GHz 開群組。Wi-Fi Direct 的 SCC 會把 P2P 群組綁到 GO 的 STA 頻道——GO 掛在 2.4GHz AP → 群組被鎖
     * 2.4GHz（~13Mbps）。故選舉時優先讓 「能跑 5GHz」的那台當 GO（見 {@link #shouldBeGroupOwner}）。未知時樂觀視為 true。
     */
    public final boolean canHost5G;

    public PairingToken(
            int version, @NonNull byte[] nonce, int goIntent, @Nullable String deviceName) {
     ...
     *
     * <p>僅用於本機身分的<b>首次生成</b>(見 {@link LocalPairing});刷新酬載請勿走這裡, 否則會連身分一起換掉——那正是「BLE
     * 掃不到/選舉打架」的根因。刷新酬載用 {@link #withPayload}。
     */
     ...
     *
     * <p>這是「刷新本機能力/姓名」的唯一正確途徑:對方在本場已掃定我的 nonce、已用我的 goIntent 算選舉,換酬載不得動到這兩者。配合 {@link
     * LocalPairing#current()} 達成「穩定身分 + 最新酬載」。
     */
     ...
     *       <p>原理:Wi-Fi Direct 的 SCC 對 GO 與 client 兩端都成立——被 2.4GHz STA 綁住的裝置 <b>只能</b>在 2.4GHz
     *       運作,無法跟去別人開在 5GHz 的群組(單無線電跨頻段做不到)。 因此讓「受限的那台」當 GO,群組就會落在 2.4GHz——一個兩端都能加入的頻段,確保連得上
     *       (代價是慢)。反之讓 5GHz 裝置當 GO 會把群組開在 5GHz,2.4GHz 那台永遠加入不了 → 連線失敗。
     *       <p>要拿到 5GHz 高速,需兩台都離開 2.4GHz(皆 {@code canHost5G==true},此項同分後群組自選 5GHz); 此時由上層({@link
     *       #shouldBeGroupOwner} 呼叫端)提示使用者。
     *   <li>goIntent 大者勝;
     *   <li>同分則 nonce 字典序大者勝。
     * </ol>
     *
