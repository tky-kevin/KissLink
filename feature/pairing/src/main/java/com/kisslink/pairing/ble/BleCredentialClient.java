 ...
 *
 * <ol>
 *   <li>把<b>自己的</b> token 寫進 {@link BleConstants#CHAR_PEER_TOKEN}(讓對方湊齊兩份 token)。
 *   <li>開啟 {@link BleConstants#CHAR_CREDENTIAL} 的 notify。
 *   <li>就緒({@link Callback#onReady}):接著由協調器依 GO 選舉結果決定 {@link #publishCredential}(本機是 GO)或等待
 *       notify({@link Callback#onCredentialReceived})。
 * </ol>
 */
public class BleCredentialClient {
                ...
                () -> {
                    ...
                    if (scanning) {
                        FlightRecorder.event(
                                TAG,
                                "BLE timeout: never saw peer advertising nonce="
                                        + scanNonceB64
    ...
    public void start(@NonNull byte[] peerNonce, @NonNull PairingToken localToken) {
        ...
        if (scanner == null) {
            callback.onError("此裝置不支援 BLE 掃描");
            return;
        }
    ...
    /**
     * 非 GO 端在 onReady(已連上、CCCD 已開)後呼叫:啟動憑證讀取後備。 若 GO 的 notify 沒到(notify-before-subscribe race /
     * 丟包),改週期性 READ credentialChar， GO 一旦 publishCredential 設好值即讀得到 → onCredentialReceived。GO
     * 端本機不需呼叫。
     */
    @SuppressLint("MissingPermission")
        ...
        credentialReadPoll =
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (attempts++ >= 15) {
                            credentialReadPoll = null;
                            return;
                        } // ~9s 後交給 coordinator watchdog
                        try {
                            gatt.readCharacteristic(credentialChar);
    ...
    public void stop() {
        ...
        if (credentialReadPoll != null) {
            main.removeCallbacks(credentialReadPoll);
            credentialReadPoll = null;
        }
        try {
    ...
    /**
     * 交握逾時（GATT 已連上但 discover/MTU/token/CCCD 卡住未達 onReady）： 主動斷掉目前 gatt（其 DISCONNECTED 因 g!=gatt
     * 會被忽略），在重試額度內換一條新連線重來； 用盡才回報錯誤。把「靜默卡住」轉成「快速重連」，多數暫時性 BLE 卡頓可自癒。
     */
    ...
    /**
     * 統一的「close 目前 gatt + 在預算內重連」：connect / handshake 兩種卡死共用。 把「靜默卡死」轉成「快速重連」，多數暫時性 BLE
     * 卡頓可自癒；用盡額度才回報錯誤。
     */
    @SuppressLint("MissingPermission")
    ...
    private void onScanRestart() {
        if (stopped || !scanning || scanner == null || scanFilters == null || scanSettings == null)
            return;
            ...
            new BluetoothGattCallback() {
                ...
                public void onConnectionStateChange(BluetoothGatt g, int status, int newState) {
                    if (newState == BluetoothGatt.STATE_CONNECTED
                        ...
                        if (!servicesDiscovered && gattAttempts < MAX_GATT_ATTEMPTS) {
                            main.postDelayed(
                                    BleCredentialClient.this::connectGatt,
                                    GATT_RETRY_BACKOFF_MS); // 早期 133 → 自動重試
                        } else {
                ...
                public void onServicesDiscovered(BluetoothGatt g, int status) {
                    if (status != BluetoothGatt.GATT_SUCCESS
                            || g.getService(BleConstants.SERVICE_UUID) == null) {
                    ...
                    peerTokenChar =
                            g.getService(BleConstants.SERVICE_UUID)
                                    .getCharacteristic(BleConstants.CHAR_PEER_TOKEN);
                    credentialChar =
                            g.getService(BleConstants.SERVICE_UUID)
                                    .getCharacteristic(BleConstants.CHAR_CREDENTIAL);
                ...
                public void onCharacteristicWrite(
                        BluetoothGatt g, BluetoothGattCharacteristic ch, int status) {
                    if (ch == null) return;
                    if (BleConstants.CHAR_PEER_TOKEN.equals(ch.getUuid())) {
                        ...
                        BluetoothGattDescriptor cccd =
                                credentialChar.getDescriptor(BleConstants.CCCD);
                        if (cccd != null) {
                ...
                public void onDescriptorWrite(
                        BluetoothGatt g, BluetoothGattDescriptor d, int status) {
                ...
                public void onCharacteristicChanged(
                        BluetoothGatt g, BluetoothGattCharacteristic ch) {
                ...
                public void onCharacteristicRead(
                        BluetoothGatt g, BluetoothGattCharacteristic ch, int status) {
                    if (ch != null
                            && BleConstants.CHAR_CREDENTIAL.equals(ch.getUuid())
                            && status == BluetoothGatt.GATT_SUCCESS) {
    ...
    private static GroupCredential parseCredential(@Nullable byte[] value) {
        ...
        try {
            return NFCCredential.fromBytes(value);
        } catch (Exception e) {
            // 解析失敗不該靜默：留下診斷，否則只會表現為「卡在連線逾時」難以定位。
            FlightRecorder.event(
                    TAG, "parseCredential failed (" + value.length + "B): " + e.getMessage());
            return null;
        }
    }
}
