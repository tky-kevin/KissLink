 ...
 *
 * <p>標準 NDEF AID {@code D2760000850101} 會與每台 Android 內建的 {@code
 * com.android.nfc.ndef_nfcee.NdefNfceeS​ervice}(「嵌入式標記」)<b>搶同一個 AID</b>; 當本 App 不在前景(無法
 * setPreferredService)時,系統無法自動二選一 → 跳出卡片模擬 衝突選擇器(Samsung 的 ConflictResolverActivity)。改用私有 AID
 * 後完全不與系統服務衝突。
 *
    ...
    public byte[] processCommandApdu(byte[] apdu, Bundle extras) {
        ...
        if (l != null) {
            try {
                ...
                Log.w(TAG, "readListener error", e);
            }
        }
