 ...
 * 環形緩衝;發生失敗時把緩衝快照落檔。<b>單一時間線</b>是重點:配對 → 選舉 → 建群 → socket
 * → 傳輸是一條連續因果鏈,交錯在同一條 timeline 才看得出跨層因果(例如「配對成功但 socket
 * 建不起來」)。事後用 {@code adb pull} 或檔案管理器取出,不必每次蹲守 adb 重現。
 *
 * <h3>為何用環形緩衝而非讀 logcat</h3>
 * 第三方 app 沒有 {@code READ_LOGS} 權限,在裝置上只讀得到殘缺的自身 log;自記緩衝完全
 * 不依賴權限,且診斷序列即使 logcat tag 關閉(預設靜默)也照樣留存。
 *
 * <h3>取出位置</h3>
 * {@code <外部檔案目錄>/diagnostics/session-<時戳>.log},例如 debug build:
...
public final class FlightRecorder {
    ...
    public static File dump(@NonNull Context ctx, @NonNull String reason) {
        try {
            ...
            rotate(dir);
            Log.i(TAG, "diagnostics dumped → " + out.getAbsolutePath()
    ...
    private static void rotate(@NonNull File dir) {
        File[] files = dir.listFiles((d, name) -> name.startsWith(DUMP_PREFIX) && name.endsWith(".log"));
