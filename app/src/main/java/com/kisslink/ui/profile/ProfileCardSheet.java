 ...
 *
 * <p>檢視模式：頭像、姓名、欄位唯讀，底部「修改 / 傳送名片」。按修改進編輯模式可換頭像、 改姓名、增刪欄位。按傳送名片 → 卡片縮小飛向對方的有機動畫後，直接傳送（無視待傳清單）。
 */
public class ProfileCardSheet extends DialogFragment {
            ...
            registerForActivityResult(
                    ...
                    uri -> {
                        ...
                        avatarCropLauncher.launch(
                                AvatarCropActivity.newIntent(requireContext(), uri));
                    });

    public static ProfileCardSheet newInstance() {
        return new ProfileCardSheet();
    }
    ...
    private void flyAndSend() {
        ...
        ObjectAnimator lift =
                ObjectAnimator.ofFloat(card, View.TRANSLATION_Y, 0f, -card.getHeight() * 1.0f);
                ...
                new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator a) {
                        ...
                        if (getActivity() instanceof Host)
                            ((Host) getActivity()).sendMyProfileCard();
    ...
    private boolean rowBlank(View row) {
        return ((EditText) row.findViewById(R.id.etFieldLabel))
                        .getText()
                        .toString()
                        .trim()
                        .isEmpty()
    ...
    private void saveAndExit() {
        ...
        for (View row : new ArrayList<>(fieldRows)) {
            ...
            String val =
                    ((EditText) row.findViewById(R.id.etFieldValue)).getText().toString().trim();
            if (l.isEmpty() && val.isEmpty()) {
    ...
    private void renderAvatar() {
        ...
        ivAvatar.setImageBitmap(
                ProfileStore.get(requireContext())
                        .loadAvatarForDisplay(requireContext(), AVATAR_DISPLAY_PX));
    }
