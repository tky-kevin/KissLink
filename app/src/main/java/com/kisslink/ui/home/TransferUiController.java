 ...
 *
 * <ul>
 *   <li>Connection-stage ticker (NFC→BLE→Wi-Fi progression labels).
 *   <li>Headline / percent / speed display helpers.
 *   <li>Peer-identity avatar rendering.
 * </ul>
 *
 * <p>HomeActivity delegates here after each {@code onSession} callback; the Activity keeps only
 * lifecycle, NFC, service binding, and user-intent forwarding.
 */
final class TransferUiController {
    ...
    /**
     * 傳輸中速度「英雄字」：大號數字（tabular）＋單位，借用 percentRow 的大字版位。 不顯示副標（傳輸細節改由列表方塊呈現）；只在進入傳輸的第一幀播一次淡入上滑，
    ...
    /**
     * 連線完成(CONNECTED)時呼叫:讓階段 ticker 把剩餘階段(含最後的「建立 TCP 通道」)依序走完、 最後一階段再停留一拍,然後才執行 {@code
     * whenDone}(切到「已連線」)。若 ticker 未在跑 (例如同裝置重貼 resume、傳輸結束回連線),則立即執行。
     */
    ...
    private final Runnable stageTick =
            new Runnable() {
                @Override
                public void run() {
    ...
    /** 顯示「載入中」大標(結尾帶會跑動的「…」),換文字時播淡入上滑,省略號更新不重播動畫。 供階段 ticker 與「重整連線」共用。 */
    ...
    /** 渲染「base + 三點」,但只讓前 1~3 點可見、其餘設為透明。 三點永遠佔位 → 文字寬度固定、置中不抖動(避免點數變化時整行左右跳)。 */
    ...
    private final Runnable ellipsisTick =
            new Runnable() {
                @Override
                public void run() {
    ...
    private String stageLabel(int step) {
        switch (step) {
            case 0:
                return context.getString(R.string.stage_nfc);
            case 1:
                return context.getString(R.string.stage_ble);
            case 2:
                return context.getString(R.string.stage_elect);
            case 3:
                return context.getString(R.string.stage_wifi);
            default:
                return context.getString(R.string.stage_tcp);
        }
    }
    ...
    /**
     * Maps a session phase to the stage ticker target index (0=NFC, 1=BLE, 2=協商主機, 3=Wi-Fi Direct,
     * 4=TCP通道).
     *
     * <p>The ticker only advances and dwells, so a jump (e.g. BLE→TCP) still walks through the
     * intermediate labels briefly — no phase needs to fire for its label to show.
     */
    static int stageTargetFor(@NonNull com.kisslink.transfer.SessionState.Phase p) {
        switch (p) {
            ...
            case SOCKETING:
                return 4;
            default:
                return 3; // CONNECTING（Wi-Fi Direct 建群/連線）
        }
    }
