 ...
 * 自動捲動是否仍生效、上次自動捲到的列——以及所有清單渲染方法收斂於此。先前這些旗標被十餘個
 * render 方法跨來跨去地讀寫，是「狀態散落＋手動同步」的漂移溫床；集中後此類別是清單顯示狀態的
 * 唯一權威，{@link HomeActivity} 只透過明確 API 委派。
 *
 * <p>持久接收清單的「資料」真相仍在 {@link HomeViewModel}（跨 Activity 重建存活）；本類別只持有
 * 「呈現」狀態（View 重建時由 sticky SessionState + VM 真相重新推導），符合 MVVM 分層。
 */
final class TransferListPresenter {
    ...
    TransferListPresenter(@NonNull MaxHeightRecyclerView rv, @NonNull HomeViewModel vm,
    ...
    // ── 呈現狀態查詢 ──────────────────────────────────────────────
    boolean isReceiveListActive() { return receiveListActive; }
    boolean isListVisible() { return rv.getVisibility() == View.VISIBLE; }
    ...
    /**
     * 離開傳輸版面（回到已連線/完成/就緒）：送出列表收合；
     * 接收列表為持久顯示（收完仍保留，供檢視/點開）→ 不收合。
     */
    void exitTransfer() {
        inTransferUi = false;
    ...
    /**
     * 把待傳清單展開成「列表方塊」（沿用待傳列設計、無標題）。只在進入傳輸時建一次——
     * 結構（檔名/大小/縮圖）在整段傳輸內不變，逐幀的進度/高亮由 {@link #updateOutgoingProgress}
     * 局部更新，避免每幀整列重綁導致縮圖閃爍。
     */
    void buildSendList() {
    ...
    void showReceiveList(@NonNull TransferProgress tp, boolean curFileDone) {
        boolean isNew = vm.upsertReceived(tp.batchId, tp.fileName, tp.totalBytes,
     ...
     * <p>由「接收完成」「待傳清單變動」「重建還原」三條路徑共用：收合是狀態(有清單+有待傳)的函數，
