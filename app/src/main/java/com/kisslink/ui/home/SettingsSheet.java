 ...
 *
 * <p>選擇後立即透過 {@link ThemePrefs#setMode} 套用並儲存偏好， AppCompatDelegate 會自動重建 Activity。
 */
public class SettingsSheet extends BottomSheetDialogFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        rowSystem.setOnClickListener(
                v -> applyMode(ThemePrefs.MODE_SYSTEM, checkSystem, checkLight, checkDark));
        rowLight.setOnClickListener(
                v -> applyMode(ThemePrefs.MODE_LIGHT, checkSystem, checkLight, checkDark));
        rowDark.setOnClickListener(
                v -> applyMode(ThemePrefs.MODE_DARK, checkSystem, checkLight, checkDark));
    }

    private void applyMode(
            int mode, ImageView checkSystem, ImageView checkLight, ImageView checkDark) {
