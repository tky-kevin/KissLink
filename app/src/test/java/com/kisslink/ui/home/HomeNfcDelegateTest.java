 ...
 * 配對前置就緒判定的單元測試。
 *
 * <p>{@link HomeNfcDelegate} 本身與 Android framework 耦合（建構子吃 AppCompatActivity、 直接呼叫 PermissionHelper
 * 靜態 API），無法在純 JUnit 下實例化。故將優先序判定抽成純函式 {@link HomeNfcDelegate#evaluateReadiness} 後在此鎖住其優先序：權限 → 無線電
 * → 定位 → 熱點。
 *
...
public class HomeNfcDelegateTest {
    ...
    public void allReady_returnsReady() {
        assertEquals(Readiness.READY, HomeNfcDelegate.evaluateReadiness(true, false, false, false));
    }
    ...
    public void missingPerms_takesPrecedenceOverEverything() {
        ...
        assertEquals(
                Readiness.NEED_PERMS, HomeNfcDelegate.evaluateReadiness(false, true, true, true));
        assertEquals(
                Readiness.NEED_PERMS,
    ...
    public void radioOff_takesPrecedenceOverLocationAndHotspot() {
        ...
        assertEquals(
                Readiness.NEED_RADIO, HomeNfcDelegate.evaluateReadiness(true, true, true, true));
        assertEquals(
                Readiness.NEED_RADIO, HomeNfcDelegate.evaluateReadiness(true, true, false, false));
    }
    ...
    public void hotspotOn_blocksOnlyWhenPermsRadioAndLocationOk() {
        assertEquals(
                Readiness.HOTSPOT_ON, HomeNfcDelegate.evaluateReadiness(true, false, false, true));
    }
}
